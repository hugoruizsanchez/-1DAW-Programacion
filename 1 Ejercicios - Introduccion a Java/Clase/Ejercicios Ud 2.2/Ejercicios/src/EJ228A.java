...
public class EJ228A {
	...
	public static void main(String[] args) {
		  ...
		  long binario_int; // En esta variable se guardará el número binario que el usuario introduzca en pantalla. 
          long binario_decimal =0; // En esta variable se guardará el resultado de binario a decimal. 
		  ...
		  case 1: // DE BINARIO A DECIMAL. 
		       ...
		       int[] binario_array = Integer.toString((int) binario_int).chars().map(c -> c-'0').toArray();// Esta expresión sirve para convertir el número entero en un array con números separados.
		...
		while (decimal_int > 2) { // Mientras el decimal_int (dividendo) NO sea igual a decimal_digito (resto)
			 ...
			 // Sumamos valor al iterador j cada vez que se repite el while, esto nos ayudará más adelante para saber cuántos numeros de decimal binario [] imprimir
					 ...
					 System.out.print("El decimal_digito es ="+decimal_digito);
					 
					 if (decimal_digito%2 ==0) { // Si el resto es par...
						 decimal_digito =0; // El digito será cero. 
						 System.out.println("Numero par, convertido en 0. ");
					 }
					 else if (decimal_digito%3==0) { // Si es impar...
						 decimal_digito =1; // El dígito será 1
						 System.out.println("Numero par, convertido en 1. ");
					 }
					
					 decimal_binario [j] = decimal_digito; // El dígito (0 o 1) se guardarán en la  posición j del array 
					 j++;
					System.out.println ("Se ha asignado "+decimal_digito+ " a decimal_binario j"+j+" que es " +decimal_binario [j]);
					 
					 decimal_int= decimal_int/2; // Dividimos entre 2 como indica la fórmula (ver anotaciones)
					 
					 System.out.println ("Decimal /2 ="+ decimal_int);
					 
			} 

if (decimal_int ==2) {
	decimal_int =10;         // Finalmente, debemos tomar de referencia el último dígito (que solo puede ser 1 o 2 y pasarlo  a binario)
}else {
	decimal_int =1;
}	
		
		System.out.print (decimal_int);
			for (int i=(j-1); i>=0; i--) { // Repetimos tantas veces como valores "j" haya, esto es, cuantos valores existan escritos. 
